...
1. The navigation buttons.<br>    1.0 Login<br>    1.2 Git<br>    1.3 New batch<br>    <br>1.4 QuickStatements 3.0
2. The new batch form<br>    2.1 Command format<br>        - CSV command format<br>        <br>- V1 command format<br>    2.2 Custom bacth name<br>    2.3 Text area for entering commands<br>    2.4  Error message
...
The L in Len stands for label while the last two letters are any valid two-letter abbreviation of any language. Therefore, Len simply means the label of the item being created or edited is written in English.
...
- /* */ Is is used to add a comment to a command. Example: Q12|P1234|Q5|!S123|Q76|/* Comment here */
...
CREATE
LAST|P1234|Q5|/* Comment here */
LAST|Len|"The label in English"
...
## 2.2 Custom bacth name

The command batch name is the second input field of the form. This field allows the user to name the bacth of commands that will be execusted. If not provided the a batch name "user:anonymous {date and time of submition}" is given as the name for the batch of commands submitted.

## 2.3 Text area for entering commands

This is the field in which the user creates, adds, changes and deletes items and its values with the format explained above.

## 2.4 Error message

The new batch page displays errors that prevents the execute button from running successfully in a position between the heading "New batch" and the form. The error only shows up in that postion on the page if there is any.
